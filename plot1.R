
setwd("C:\\Users\\hassan\\Desktop\\TEST\\data")
path<-"C:\\Users\\hassan\\Desktop\\TEST\\data"
dir()


# READING  The rds file
scc <- readRDS("./Source_Classification_Code.rds")
nei<-readRDS("./summarySCC_PM25.rds")
#head(rds)
#names(nei)

s1<-subset(nei,year=="1999") ;m1<-sum(s1$Emissions,na.rm=TRUE)
s2<-subset(nei,year=="2002") ;m2<-sum(s2$Emissions,na.rm=TRUE)
s3<-subset(nei,year=="2005") ;m3<-sum(s3$Emissions,na.rm=TRUE)
s4<-subset(nei,year=="2008") ;m4<-sum(s4$Emissions,na.rm=TRUE)
maxdata<-data.frame(year=as.numeric(c("1999","2002","2005","2008")),total=c(m1,m2,m3,m4))


# 
# 
plot.new( xaxt = "n", yaxt = "n")  #activate the graphics device in R
# 
with(maxdata,plot(year,total,type="b",pch=20,cex=3,col=y,lwd=1,
                 main="total emissions from PM2.5",
                 xlab="",
                 ylab="",
                 yaxt='n',xaxt='n'))

axis(1, at=maxdata$year)
title(xlab="year", cex.lab = 1.2,col.lab="blue")
axis(2, at=maxdata$total,las=2,
labels=format(maxdata$total, scientific=TRUE,digits=2))
title(ylab="Total amount PM2.5", cex.lab = 1.2,line = 5.0,col.lab="blue")

## line>0 move the label farther and line<0 move it to the inner side of the plot

dev.copy(png,file = "plot1.png",width = 480, height = 480)
dev.off()

